...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		for c, bitrate := range weightInfo {
		...
		weightSum := 0.0
		for c, weight := range weightInfo_new {
			if weight > 5 {
				weight = 5
				weightInfo_new[c] = 5
			} else if weight < 0.5 {
		...
		bandwidth_default := float64(1024*768) / 6 * 9
		bandwidth_r := bandwidth_default * 2 / 9 //in Byte (1024*768 = 6 Mbps)
