...
func main() {
	...
	i2, i22 := 1, 1
	var qoe_3 [50][2]float64
	i3, i33 := 1, 1
	u1_avg, u2_avg := 0.0, 0.0
	...
	u3_sum_before, u3_avg_before := 0.0, 0.0
	u3_sum_after, u3_avg_after := 0.0, 0.0
	// Timer_before := 0.0
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if req_data_map["did"] == "ppd001" || req_data_map["did"] == "ppd004" {
				// rebuffering = rebuffering -stay time
				rebuffering = rebuffering - (40 - Timer)
				//C1 lost connection when Timer > 40
				if Timer > 40 {
					if lastBuffer_if == 0 {
						lastBuffer = BufferTime
						lastBuffer_if = 1
					}
					CurrentBitrate = 0
					smoothness = 0
					// DownloadTime is current handover time + average DownloatTime
					// So DownloadTime = (20 - (Timer-40)) +2
					// Buffertime = lastBuffer-(Timer-40)
					// rebuffering = ((20 - (Timer - 40)) + 2) - (lastBuffer - (Timer - 40)) + (20 - (Timer - 40))
					if lastBuffer-(Timer-40) > 0 {
						rebuffering = ((20 - (Timer - 40)) + 2) - (lastBuffer - (Timer - 40))
					} else {
						rebuffering = 2
					}

				}
			} else if req_data_map["did"] == "ppd002" || req_data_map["did"] == "ppd005" {
				// rebuffering = rebuffering - stay time
			...
			} else if req_data_map["did"] == "ppd003" || req_data_map["did"] == "ppd006" {
				// rebuffering = rebuffering - stay time
				rebuffering = rebuffering - (50 - Timer)
				//C3 lost connection when Timer>50
				if Timer > 50 {
					if lastBuffer_if3 == 0 {
						lastBuffer3 = BufferTime
						lastBuffer_if3 = 1
					}
					CurrentBitrate = 0
					smoothness = 0
					// DownloadTime is current handover time + average DownloatTime
					// So DownloadTime = (25 - (Timer-50)) +2
					// Buffertime = lastBuffer3-(Timer-50)
					if lastBuffer3-(Timer-50) > 0 {
						rebuffering = ((25 - (Timer - 50)) + 2) - (lastBuffer3 - (Timer - 50))
					} else {
						rebuffering = 2
					}
				}
			...
			if req_data_map["did"] == "ppd001" {
				if Timer != 0 && Timer < 60 && Timer != qoe_1[i1-1][0] {
					...
					if Timer < 40 {
						i11 = i11 + 1
						u1_sum_before = u1_sum_before + utility
				...
				if Timer != 0 && Timer < 90 && Timer != qoe_2[i2-1][0] {
					...
					if Timer < 60 {
						i22 = i22 + 1
						u2_sum_before = u2_sum_before + utility
			...
			} else if req_data_map["did"] == "ppd003" {
				if Timer != 0 && Timer < 75 && Timer != qoe_3[i3-1][0] {
					qoe_3[i3][0] = Timer
					qoe_3[i3][1] = utility
					i3 = i3 + 1
					if Timer < 50 {
						i33 = i33 + 1
						u3_sum_before = u3_sum_before + utility
						u3_avg_before = u3_sum_before / float64(i3)
					} else if Timer >= 50 {
						u3_sum_after = u3_sum_after + utility
						u3_avg_after = u3_sum_after / float64(i3-i33)
					}
				}
			}
			// plot
			...
			if plot_if == 1 {
				...
				u3_avg = u3_avg_before
				// u1_avg = u1_avg_before*2/3 + u1_avg_after*1/3
				// u2_avg = u2_avg_before*2/3 + u2_avg_after*1/3
				// u3_avg = u3_avg_before*2/3 + u3_avg_after*1/3

				str1 := "client1_QoE_avg=" + strconv.FormatFloat(u1_avg, 'f', 0, 64)
				str2 := "client2_QoE_avg=" + strconv.FormatFloat(u2_avg, 'f', 0, 64)
				str3 := "client3_QoE_avg=" + strconv.FormatFloat(u3_avg, 'f', 0, 64)
				plotutil.AddLinePoints(p, str1, pts1, str2, pts2, str3, pts3)

				p.Save(4*vg.Inch, 4*vg.Inch, "QoE1and2and3.png")
		...
		bandwidth_default := float64(1024*768) / 6 * 18
		...
		for clientName, weight := range weightInfo_new {
			...
			bandwidth_r = bandwidth_default * 1
			...
			// }
			// if Timer_global > 60 {
			// 	bandwidth_r = bandwidth_default * 1 / 3
			// }
