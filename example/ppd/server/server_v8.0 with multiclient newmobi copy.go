...
func main() {
	...
	var weightInfo_p sync.Map

	var weightInfo sync.Map
	var weightInfo_new sync.Map
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if Timer_global > 90 && Timer_global < 130 {
				...
				weightInfo.Delete("ppd001")
				weightInfo.Delete("ppd004")
				weightInfo.Delete("ppd007")
				weightInfo.Delete("ppd0010")
				weightInfo.Delete("ppd0013")
				...
				weightInfo_new.Delete("ppd001")
				weightInfo_new.Delete("ppd004")
				weightInfo_new.Delete("ppd007")
				weightInfo_new.Delete("ppd0010")
				weightInfo_new.Delete("ppd0013")
			}
			...
			if Timer_global > 90/(150.0/260.0) && Timer_global < 130/(150.0/260.0) {
				...
				weightInfo.Delete("ppd002")
				weightInfo.Delete("ppd005")
				weightInfo.Delete("ppd008")
				weightInfo.Delete("ppd0011")
				weightInfo.Delete("ppd0014")
				...
				weightInfo_new.Delete("ppd002")
				weightInfo_new.Delete("ppd005")
				weightInfo_new.Delete("ppd008")
				weightInfo_new.Delete("ppd0011")
				weightInfo_new.Delete("ppd0014")
			}
			...
			if Timer_global > 260 {
				...
				avg_qoe_total, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_qoe_total/10), 64)
				avg_qoe_total_serverPush, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_qoe_total_serverPush/10), 64)
				fmt.Println("avg_qoe_total:", avg_qoe_total, "avg_qoe_total_serverPush", avg_qoe_total_serverPush)
			...
			weightInfo.Store(dash_data_split[0], weight)
		...
		weightInfo.Range(func(k, v interface{}) bool {
			weight := v.(float64)
			weightSum = weightSum + weight
			clientNum = clientNum + 1
			return true
			...
			if flag == true {
				...
				weightInfo.Range(func(k, v interface{}) bool {
					...
					if weight < (weightSum/clientNum)*0.6 {
						weightInfo_new.Store(c, ((weightSum / clientNum) * 0.6))
					} else {
						weightInfo_new.Store(c, weight)
					}
					return true
				})
		...
		weightInfo_new.Range(func(k, v interface{}) bool {
			weight := v.(float64)
			weightSum_new = weightSum_new + weight
			return true
		})
		...
		weightInfo_new.Range(func(k, v interface{}) bool {
			...
			weightInfo_p.Store(clientName, wp)
