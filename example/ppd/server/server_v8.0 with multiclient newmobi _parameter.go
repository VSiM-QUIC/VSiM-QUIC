...
func main() {
	...
	clientBit_chunk := make(map[string]map[string]float64)
	clientBuff_chunk := make(map[string]map[string]float64)
	// client1_reqTime:= []
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			utility := CurrentBitrate - beta*rebuffering - lambda*smoothness
			utility, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", utility), 64)
			qoe_new := CurrentBitrate - 10*(DownloadTime-BufferTime) - 0.1*smoothness
			...
			addClient_chunk = func(client string) {
				...
				if exist2 {
					...
					clientBit_chunk[client][chunkNum] = CurrentBitrate
					clientBuff_chunk[client][chunkNum] = BufferTime
				} else {
					timeQoE := make(map[string]float64)
					timeQoE[chunkNum] = qoe_new
					clientQoE_chunk[client] = timeQoE
					timeBit := make(map[string]float64)
					timeBit[chunkNum] = CurrentBitrate
					clientBit_chunk[client] = timeBit
					timeBuff := make(map[string]float64)
					timeBuff[chunkNum] = BufferTime
					clientBuff_chunk[client] = timeBuff
				}
			}
			if ok2 {
				...
				clientBit_chunk[req_data_map["did"]][chunkNum] = CurrentBitrate
				clientBuff_chunk[req_data_map["did"]][chunkNum] = BufferTime
				...
				timeBit := make(map[string]float64)
				timeBit[chunkNum] = CurrentBitrate
				clientBit_chunk[req_data_map["did"]] = timeBit
				timeBuff := make(map[string]float64)
				timeBuff[chunkNum] = BufferTime
				clientBuff_chunk[req_data_map["did"]] = timeBuff
			}
			...
			for client := range clientQoE_chunk {
				...
				for time := range clientQoE_chunk[client] {
					...
					bit_sum = bit_sum + clientBit_chunk[client][time]
					bit_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", bit_sum), 64)
					buff_sum = buff_sum + clientBuff_chunk[client][time]
					buff_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", buff_sum), 64)
				}
				...
				if client == "ppd001" || client == "ppd004" || client == "ppd007" || client == "ppd0010" || client == "ppd0013" {
					if Timer_global <= 90 {
						...
						countNum_Qoe["bit_sum"] = bit_sum
						countNum_Qoe["buff_sum"] = buff_sum
				...
				bit_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", bit_sum/t_count), 64)
				countNum_Qoe["bit_avg"] = bit_avg
				buff_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", buff_sum/t_count), 64)
				countNum_Qoe["buff_avg"] = buff_avg
			...
			if Timer_global > 260 {
				...
				for client := range clientQoE_chunkNum_totalQoe {
					if client == "ppd001" || client == "ppd004" || client == "ppd007" || client == "ppd0010" || client == "ppd0013" {
						...
						t_count_BitBuff := clientQoE_chunkNum_totalQoe[client]["t_count"] + clientQoE_chunkNum_totalQoe2[client]["t_count"]
						bit_sum := clientQoE_chunkNum_totalQoe[client]["bit_sum"] + clientQoE_chunkNum_totalQoe2[client]["bit_sum"]
						bit_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", bit_sum), 64)
						buff_sum := clientQoE_chunkNum_totalQoe[client]["buff_sum"] + clientQoE_chunkNum_totalQoe2[client]["buff_sum"]
						buff_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", buff_sum), 64)
						...
						bit_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", bit_sum/t_count_BitBuff), 64)
						countNum_Qoe["bit_avg"] = bit_avg
						buff_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", buff_sum/t_count_BitBuff), 64)
						countNum_Qoe["buff_avg"] = buff_avg
						countNum_Qoe["avg"] = avg
						...
						addtional_bit_sum := clientQoE_chunkNum_totalQoe2[client]["bit_sum"]
						addtional_bit_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", addtional_bit_sum), 64)
						addtional_buff_sum := clientQoE_chunkNum_totalQoe2[client]["buff_sum"]
						addtional_buff_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", addtional_buff_sum), 64)
						...
						addtional_bit_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", addtional_bit_sum/clientQoE_chunkNum_totalQoe2[client]["t_count"]), 64)
						addtional_buff_avg, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", addtional_buff_sum/clientQoE_chunkNum_totalQoe2[client]["t_count"]), 64)
						additional_countNum_Qoe["bit_sum"] = addtional_bit_sum
						additional_countNum_Qoe["buff_sum"] = addtional_buff_sum
						additional_countNum_Qoe["bit_avg"] = addtional_bit_avg
						additional_countNum_Qoe["buff_avg"] = addtional_buff_avg
				...
				for client := range clientQoE_chunkNum_totalQoe3 {
					if client == "ppd002" || client == "ppd005" || client == "ppd008" || client == "ppd0011" || client == "ppd0014" {
						...
						t_count_BitBuff := clientQoE_chunkNum_totalQoe3[client]["t_count"] + clientQoE_chunkNum_totalQoe4[client]["t_count"]
						bit_sum := clientQoE_chunkNum_totalQoe3[client]["bit_sum"] + clientQoE_chunkNum_totalQoe4[client]["bit_sum"]
						bit_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", bit_sum), 64)
						buff_sum := clientQoE_chunkNum_totalQoe3[client]["buff_sum"] + clientQoE_chunkNum_totalQoe4[client]["buff_sum"]
						buff_sum, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", buff_sum), 64)
				...
				fmt.Println("----------------------------------")
				fmt.Println("qoe beta=10")
				fmt.Println(label, "utility beta=", beta, "lambda=", lambda, "update frequency:", updateFreq)
				...
				fmt.Println("----------------------------------")
				avg_qoe_total := 0.0
				avg_qoe_total_10 := 0.0
				avg_qoe_total_serverPush := 0.0
				avg_qoe_total_serverPush_10 := 0.0
				clientNum := 0.0
				clientNum_10 := 0.0
				// if label == "Vsim" {
				for client := range clientQoE_chunkNum_totalQoe_final {
					match10, _ := regexp.MatchString("ppd00*", client)
					if match10 {
						fmt.Println(client, "avg/SP:", clientQoE_chunkNum_totalQoe_final[client]["avg"], "/", clientQoE_chunkNum_totalQoe_final[client]["avg_serverPush"],
							"qoe_sum/SP:", clientQoE_chunkNum_totalQoe_final[client]["qoe_sum"], "/", clientQoE_chunkNum_totalQoe_final[client]["qoe_sum_serverPush"], "t_count:", clientQoE_chunkNum_totalQoe_final[client]["t_count"])
						avg_qoe_total = avg_qoe_total + clientQoE_chunkNum_totalQoe_final[client]["avg"]
						avg_qoe_total_10 = avg_qoe_total_10 + clientQoE_chunkNum_totalQoe_final[client]["avg"]
						...
						if ok {
							...
							avg_qoe_total_serverPush_10 = avg_qoe_total_serverPush_10 + clientQoE_chunkNum_totalQoe_final[client]["avg_serverPush"]
						} else {
							avg_qoe_total_serverPush = avg_qoe_total_serverPush + clientQoE_chunkNum_totalQoe_final[client]["avg"]
							avg_qoe_total_serverPush_10 = avg_qoe_total_serverPush_10 + clientQoE_chunkNum_totalQoe_final[client]["avg"]
						}
						clientNum = clientNum + 1
						clientNum_10 = clientNum_10 + 1
					}
				}
				for client := range clientQoE_chunkNum_totalQoe_final {
					match15, _ := regexp.MatchString("client*", client)
					if match15 {
						fmt.Println(client, "avg/SP:", clientQoE_chunkNum_totalQoe_final[client]["avg"], "/", clientQoE_chunkNum_totalQoe_final[client]["avg_serverPush"],
							"qoe_sum/SP:", clientQoE_chunkNum_totalQoe_final[client]["qoe_sum"], "/", clientQoE_chunkNum_totalQoe_final[client]["qoe_sum_serverPush"], "t_count:", clientQoE_chunkNum_totalQoe_final[client]["t_count"])
						avg_qoe_total = avg_qoe_total + clientQoE_chunkNum_totalQoe_final[client]["avg"]
				...
				avg_qoe_total_10, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_qoe_total_10/clientNum_10), 64)
				fmt.Println("avg_qoe_total:", avg_qoe_total, "clientNum", clientNum, "avg_qoe_total_10:", avg_qoe_total_10, clientNum_10)
				avg_qoe_total_serverPush, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_qoe_total_serverPush/clientNum), 64)
				avg_qoe_total_serverPush_10, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_qoe_total_serverPush_10/clientNum_10), 64)
				fmt.Println("avg_qoe_total_SP:", avg_qoe_total_serverPush, "clientNum", clientNum, "avg_qoe_total_SP10:", avg_qoe_total_serverPush_10, clientNum_10)
				// }
				// if label == "Cubic" {
				// 	for client := range clientQoE_chunkNum_totalQoe_final {
				...
				clientNum = 0.0
				for client := range clientQoE_chunkNum_totalQoe_final {
					fmt.Println(client, "bit_sum:", clientQoE_chunkNum_totalQoe_final[client]["bit_sum"], "bit_avg:", clientQoE_chunkNum_totalQoe_final[client]["bit_avg"],
						"buff_sum:", clientQoE_chunkNum_totalQoe_final[client]["buff_sum"], "buff_avg", clientQoE_chunkNum_totalQoe_final[client]["buff_avg"])
					avg_bit_total = avg_bit_total + clientQoE_chunkNum_totalQoe_final[client]["bit_avg"]
					avg_buff_total = avg_buff_total + clientQoE_chunkNum_totalQoe_final[client]["buff_avg"]

					clientNum = clientNum + 1
				}
				avg_bit_total, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_bit_total/clientNum), 64)
				fmt.Println("avg_bit_total:", avg_bit_total, "clientNum", clientNum)
				avg_buff_total, _ = strconv.ParseFloat(fmt.Sprintf("%.2f", avg_buff_total/clientNum), 64)
				fmt.Println("avg_buff_total:", avg_buff_total, "clientNum", clientNum)
		...
		updateFreq = 8.0 //每4秒更新weight
		timer1 := Timer_global / updateFreq
