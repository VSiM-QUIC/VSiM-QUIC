...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if req_data_map["did"] == "ppd002" || req_data_map["did"] == "ppd005" || req_data_map["did"] == "ppd008" || req_data_map["did"] == "ppd0011" {
				// rebuffering = rebuffering -stay time
				rebuffering = rebuffering - (60 - Timer)
				//C1 lost connection when Timer > 40
			}
			if req_data_map["did"] == "ppd003" || req_data_map["did"] == "ppd006" || req_data_map["did"] == "ppd009" || req_data_map["did"] == "ppd012" {
				// rebuffering = rebuffering -stay time
			...
			if ok {
				clientQoE[req_data_map["did"]][Timer_global] = qoe_our
			} else {
				timeQoE := make(map[float64]float64)
				timeQoE[Timer_global] = qoe_our
		...
		bandwidth_default := float64(1024*768) / 6 * 24
