...
app.controller('DashController', function ($scope, sources, contributors, dashifTestVectors) {
    $scope.selectedItem = {
        // url: 'http://localhost:6060/media/bbb_30fps/bbb_30fps.mpd'
        url: 'http://localhost:6060/media/BigBuckBunny/4sec/BigBuckBunny_4s_simple_2014_05_09.mpd'
    };
    ...
    $scope.updateMetricsInterval = 1000; //ms
    ...
    $scope.videoCurrentBitrate = 0;
    $scope.videoPreviousBitrate = 0;
    $scope.videoTime = 0;
    ...
    $scope.cmcdEnabled = true;
    $scope.cmcdSessionId = '1001';
    $scope.cmcdDeviceId = 'ppd001'
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.streamInfo) {
            ...
            var bitrate = repSwitch ? Math.round(dashAdapter.getBandwidthForRepresentation(repSwitch.to, periodIdx) / 1024) : NaN;
            ...
            if (httpMetrics) {
                ...
                $scope[type + 'Download'] = httpMetrics.download[type].average.toFixed(2)
                ...
                $scope[type + 'DownloadAvg'] = httpMetrics.download[type].average.toFixed(3)
            }


            $scope.yuhan = 5
            if (type === 'video') {
                $.getJSON("app/client_data.json", function (json) {
                    $.each(json, function (key, val) {
                        $scope[key] = val
                    });
                    //console.log(mobility_data); // this will show the info it in firebug console
                });


                if ($scope.videoCurrentBitrate != $scope.videoBitrate) {
                    $scope.videoPreviousBitrate = $scope.videoCurrentBitrate
                    $scope.videoCurrentBitrate = $scope.videoBitrate
                }
                if ($scope.videoPendingIndex == $scope.videoIndex) {
                    $scope.videoPreviousBitrate = $scope.videoCurrentBitrate
                }
                console.log("Hello pb " + $scope.videoPreviousBitrate + "  cb " + $scope.videoCurrentBitrate + "  pq " + $scope.videoIndex + "  cq " + $scope.videoPendingIndex);
            }
            $scope.videoTime = getTimeForPlot()

            $scope.player.updateSettings({
                'streaming': {
                    'stableBufferTime': 50,
                    'cmcd': {
                        'cid': $scope.cmcdDeviceId + '_' + $scope.videoDownloadAvg
                            + '_' + $scope.videoBufferLength + '_' + $scope.videoCurrentBitrate
                            + '_' + $scope.videoPreviousBitrate + '_' + $scope.videoBitrate
                            + '_' + $scope.location + '_' + $scope.speed + '_' + $scope.direction
                            + '_' + $scope.videoTime
                    }
                }
            });
