...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		// parse the client info from URL and save it to req_data_map
		...
		// clean the parsed data to get the useful infomation
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			// plot
			if Timer > 80 && plot_if == 0 && plot_down == 0 {
				plot_if = 1
			}
			...
			if plot_if == 1 {
				...
				// u1_avg = u1_avg_before*0.5 + u1_avg_after*0.5
				// u2_avg = u2_avg_before*0.5 + u2_avg_after*0.5
		...
		bandwidth_default := float64(1024*768) / 6 * 6
