	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if req_data_map["did"] == "ppd001" {
				if Timer < 30 {
					...
					rebuffering = rebuffering - (90 - Timer)
				}
				// rebuffering = rebuffering -stay time
				// rebuffering = rebuffering - 60
				//C1 staytime change from 30s

			} else if req_data_map["did"] == "ppd002" {
				// rebuffering = rebuffering - stay time
				// max staytime = 60
				//C2 stay time change from 90s
				rebuffering = rebuffering - 60
			...
			if req_data_map["did"] == "ppd001" {
				if Timer != 0 && Timer < 90 && Timer != qoe_1[i1-1][0] {
					qoe_1[i1][0] = Timer
					qoe_1[i1][1] = utility
					i1 = i1 + 1
					if Timer < 90 {
			...
			// plot
			if Timer > 90 && plot_if == 0 && plot_down == 0 {
				plot_if = 1
			}
		...
		for clientName, weight := range weightInfo_new {
			...
			if clientName == "ppd001" {
				w_1[i1-1][0] = fmt.Sprintf("%.2f", Timer_global)
				w_1[i1-1][1] = fmt.Sprintf("%.2f", weightPercentage)
			}
			if clientName == "ppd002" {
				w_2[i2-1][0] = fmt.Sprintf("%.2f", Timer_global)
				w_2[i2-1][1] = fmt.Sprintf("%.2f", weightPercentage)
			}
