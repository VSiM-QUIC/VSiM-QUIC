...
func main() {
	...
	// Timer_before := 0.0
	var qoe_1 [100][3]float64
	i1 := 1
	var qoe_2 [100][3]float64
	i2 := 1
	var qoe_3 [100][3]float64
	i3 := 1
	var qoe_4 [100][3]float64
	i4 := 1
	plot_if := 0
	plot_down := 0
	// plot_mid := true
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if req_data_map["did"] == "ppd001" && timer_start {
				...
				Timer_global, _ = strconv.ParseFloat(fmt.Sprintf("%.0f", Timer_global), 64)
			}
			...
			if req_data_map["did"] == "ppd001" {
				if Timer_global != 0 && Timer_global < 120 && Timer_global != qoe_1[i1-1][0] {
					qoe_1[i1][0] = Timer_global
					qoe_1[i1][1] = qoe_new
					qoe_1[i1][2] = weightInfo_p[req_data_map["did"]]
					i1 = i1 + 1
					fmt.Println(qoe_1)
				}
			...
			if req_data_map["did"] == "ppd002" {
				if Timer_global != 0 && Timer_global < 120 && Timer_global != qoe_2[i2-1][0] {
					qoe_2[i2][0] = Timer_global
					qoe_2[i2][1] = qoe_new
					qoe_2[i2][2] = weightInfo_p[req_data_map["did"]]
					i2 = i2 + 1
				}
			}
			if req_data_map["did"] == "ppd003" {
				if Timer_global != 0 && Timer_global < 120 && Timer_global != qoe_3[i3-1][0] {
					qoe_3[i3][0] = Timer_global
					qoe_3[i3][1] = qoe_new
					qoe_3[i3][2] = weightInfo_p[req_data_map["did"]]
					i3 = i3 + 1
				}
			}
			if req_data_map["did"] == "ppd004" {
				if Timer_global != 0 && Timer_global < 120 && Timer_global != qoe_4[i4-1][0] {
					qoe_4[i4][0] = Timer_global
					qoe_4[i4][1] = qoe_new
					qoe_4[i4][2] = weightInfo_p[req_data_map["did"]]
					i4 = i4 + 1
				}
			}
			...
			if Timer_global > 120 {
				...
				if plot_if == 1 {
					...
					p2, _ := plot.New()
					p2.Title.Text = "QoE_Client"
					p2.X.Label.Text = "Time"
					p2.Y.Label.Text = "Bandwidth Share"
					// p2.Y.Max = 1.0

					pts1 := make(plotter.XYs, i1)
					pts1_2 := make(plotter.XYs, i1)
					for i := range pts1 {
						// pts1[i].X = float64(i)
						pts1[i].X = qoe_1[i][0]
						pts1[i].Y = qoe_1[i][1]
						pts1_2[i].X = qoe_1[i][0]
						pts1_2[i].Y = qoe_1[i][2]
					}
					pts2 := make(plotter.XYs, i2)
					pts2_2 := make(plotter.XYs, i2)
					for i := range pts2 {
						// pts2[i].X = float64(i)
						pts2[i].X = qoe_2[i][0]
						pts2[i].Y = qoe_2[i][1]
						pts2_2[i].X = qoe_2[i][0]
						pts2_2[i].Y = qoe_2[i][2]
					}
					pts3 := make(plotter.XYs, i3)
					pts3_2 := make(plotter.XYs, i3)
					for i := range pts3 {
						// pts1[i].X = float64(i)
						pts3[i].X = qoe_3[i][0]
						pts3[i].Y = qoe_3[i][1]
						pts3_2[i].X = qoe_3[i][0]
						pts3_2[i].Y = qoe_3[i][2]
					}
					pts4 := make(plotter.XYs, i4)
					pts4_2 := make(plotter.XYs, i4)
					for i := range pts2 {
						// pts2[i].X = float64(i)
						pts4[i].X = qoe_4[i][0]
						pts4[i].Y = qoe_4[i][1]
						pts4_2[i].X = qoe_4[i][0]
						pts4_2[i].Y = qoe_4[i][2]
					}
					...
					str3 := "client3"
					str4 := "client4"
					plotutil.AddLinePoints(p, str1, pts1, str2, pts2, str3, pts3, str4, pts4)
					plotutil.AddLinePoints(p2, str1, pts1_2, str2, pts2_2, str3, pts3_2, str4, pts4_2)
					p.Save(4*vg.Inch, 4*vg.Inch, "QoE4clients.png")
					p2.Save(4*vg.Inch, 4*vg.Inch, "Bandwidth4clients.png")

					fmt.Println("plot saved")
					fmt.Println("pts1", pts1)
					fmt.Println("pts2", pts2)
					fmt.Println("pts3", pts3)
					fmt.Println("pts4", pts4)
					fmt.Println("pts1_2", pts1_2)
					fmt.Println("pts2_2", pts2_2)
					fmt.Println("pts3_2", pts3_2)
					fmt.Println("pts4_2", pts4_2)
