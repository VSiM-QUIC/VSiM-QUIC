	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(dash_data_split) > 3 && dash_data_split[0] == req_data_map["did"] {
			...
			if req_data_map["did"] == "ppd001" || req_data_map["did"] == "ppd004" || req_data_map["did"] == "ppd007" || req_data_map["did"] == "ppd0010" {
				...
				} else if Timer_global <= 180 {
					stayTime := 180 - Timer_global
					if stayTime > 60 {
						stayTime = 60
					}
					rebuffering = rebuffering - stayTime
				...
				// rebuffering = rebuffering - stay time
				if Timer_global <= 90 {
					stayTime := 100 - Timer_global
					if stayTime > 60 {
						stayTime = 60
					}
					rebuffering = rebuffering - stayTime
				} else if Timer_global <= 180 {
					stayTime := 190 - Timer_global
					if stayTime > 60 {
						stayTime = 60
					}
					rebuffering = rebuffering - stayTime
				...
				// rebuffering = rebuffering - stay time
				if Timer_global <= 90 {
					stayTime := 110 - Timer_global
					if stayTime > 60 {
						stayTime = 60
					}
					rebuffering = rebuffering - stayTime
				} else if Timer_global <= 180 {
					stayTime := 200 - Timer_global
					if stayTime > 60 {
						stayTime = 60
					}
					rebuffering = rebuffering - stayTime
			...
			if Timer > 180 {
				...
				fmt.Println("avg_qoe_total:", avg_qoe_total)
		...
		bandwidth_default := float64(1024*768) / 6 * 2.5
